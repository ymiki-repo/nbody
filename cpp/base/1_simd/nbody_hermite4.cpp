...
#pragma omp parallel for
  for (type::int_idx i0 = 0U; i0 < Ni; i0 += type::N_simd_fp_l) {
#pragma omp simd
    for (type::int_idx i1 = 0U; i1 < type::N_simd_fp_l; i1++) {
      const auto ii = i0 + i1;
      // initialization
...
static inline void allocate_Nbody_particles(
  ...
  if ((num % type::N_simd_fp_l) != 0U) {
    size += static_cast<size_t>(type::N_simd_fp_l - (num % type::N_simd_fp_l));
  }
