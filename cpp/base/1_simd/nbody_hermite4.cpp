...
#pragma omp parallel for
  for (std::remove_const_t<decltype(Ni)> i0 = 0U; i0 < Ni; i0 += type::N_simd_fp_l) {
#pragma omp simd
    for (decltype(i0) i1 = 0U; i1 < type::N_simd_fp_l; i1++) {
      const auto ii = i0 + i1;
      // initialization
...
static inline auto set_time_step(const type::int_idx num, type::nbody &body, const type::fp_m time_pres, const type::fp_m time_sync) {
  ...
  if (time_next < time_sync) {
    ...
    for (std::remove_const_t<decltype(num)> ii = type::N_simd_fp_l; ii < num; ii += type::N_simd_fp_l) {
...
static inline void allocate_Nbody_particles(
  ...
  // number of arrays should be multiple of type::N_simd_fp_l for safety
  auto size = static_cast<size_t>(num);
  if ((num % type::N_simd_fp_l) != 0U) {
    size += static_cast<decltype(size)>(type::N_simd_fp_l - (num % type::N_simd_fp_l));
  }
