...
// #define USE_OPENACC_AS_PRAGMA_OFFLOAD
#define USE_OPENMP_TARGET_AS_PRAGMA_OFFLOAD
...
static inline void calc_acc(
  ...
  OFFLOAD_LOOP(OPTARG_ENHANCE_SIMD OPTARG_NUM_THREADS(NTHREADS))
...
static inline void trim_acc(const type::int_idx Ni, type::acceleration *__restrict acc, type::jerk *__restrict jerk
  ...
  OFFLOAD_LOOP(OPTARG_ENHANCE_SIMD)
  for (std::remove_const_t<decltype(Ni)> ii = 0U; ii < Ni; ii++) {
    // initialization
    auto ai = acc[ii];
...
static inline void sort_particles(const type::int_idx num, type::int_idx *__restrict tag, type::nbody *__restrict src, type::nbody *__restrict dst) {
  // sort particle time
  OFFLOAD_MEMCPY_D2H(src->nxt [0:num])
  ...
  OFFLOAD_MEMCPY_H2D(tag [0:num])
  OFFLOAD_LOOP(OPTARG_ENHANCE_SIMD)
#else   // EXEC_SMALL_FUNC_ON_HOST
  OFFLOAD_MEMCPY_D2H(src->pos [0:num], src->vel [0:num], src->acc [0:num], src->jrk [0:num], src->prs [0:num], src->idx [0:num])
  ...
  OFFLOAD_MEMCPY_H2D(dst->pos [0:num], dst->vel [0:num], dst->acc [0:num], dst->jrk [0:num], dst->prs [0:num], dst->nxt [0:num], dst->idx [0:num])
#endif  // EXEC_SMALL_FUNC_ON_HOST
...
static inline auto set_time_step(const type::int_idx num, type::nbody &body, const type::fp_m time_pres, const type::fp_m time_sync) {
  auto Ni = num;
  OFFLOAD_MEMCPY_D2H(body.nxt [0:num])
...
#ifdef EXEC_SMALL_FUNC_ON_HOST
  OFFLOAD_MEMCPY_H2D(body.nxt [0:num])
#endif  // EXEC_SMALL_FUNC_ON_HOST
...
#ifdef EXEC_SMALL_FUNC_ON_HOST
  OFFLOAD_MEMCPY_H2D(body.prs [0:num], body.nxt [0:num])
#endif  // EXEC_SMALL_FUNC_ON_HOST
}
...
auto main([[maybe_unused]] const int32_t argc, [[maybe_unused]] const char *const *const argv) -> int32_t {
  ...
  for (std::remove_const_t<decltype(num_bin)> ii = 0; ii < num_bin; ii++) {
    ...
    OFFLOAD_MALLOC(body0.pos [0:num], body0.vel [0:num], body0.acc [0:num], body0.jrk [0:num], body0.prs [0:num], body0.nxt [0:num], body0.idx [0:num])
    OFFLOAD_MALLOC(body1.pos [0:num], body1.vel [0:num], body1.acc [0:num], body1.jrk [0:num], body1.prs [0:num], body1.nxt [0:num], body1.idx [0:num])
    OFFLOAD_MALLOC(tag [0:num])
...
#pragma omp parallel for
    ...
    OFFLOAD_MEMCPY_H2D(body0.pos [0:num], body0.vel [0:num], body0.idx [0:num])
    ...
    OFFLOAD_MEMCPY_D2H(body0.acc [0:num], body0.jrk [0:num])
    ...
    while (present < snp_fin) {
      ...
      if (present > previous) {
        ...
        OFFLOAD_MEMCPY_D2H(body0.pos [0:num], body0.vel [0:num], body0.acc [0:num], body0.jrk [0:num], body0.idx [0:num])
        io::write_snapshot(num, body0.pos, body0.vel, body0.acc, body0.jrk, body0.idx, file.c_str(), present, time, error);
  ...
  io::write_log(argv[0], elapsed, iter, num, file.c_str());
    ...
    // memory deallocation
    OFFLOAD_FREE(body0.pos [0:num], body0.vel [0:num], body0.acc [0:num], body0.jrk [0:num], body0.prs [0:num], body0.nxt [0:num], body0.idx [0:num])
    OFFLOAD_FREE(body1.pos [0:num], body1.vel [0:num], body1.acc [0:num], body1.jrk [0:num], body1.prs [0:num], body1.nxt [0:num], body1.idx [0:num])
    OFFLOAD_FREE(tag [0:num])
