...
mpiexec -machinefile ${PJM_O_NODEINF} -n ${PJM_MPI_PROC} sh/wrapper/mpi_matplotlib.sh --wrapper-Nprocs_node=${MPI_PROC_NODE} --wrapper-Nprocs_socket=${MPI_PROC_SOCKET} --wrapper-mpl_cfg_dir=$MPL_CFG_DIR julia jl/plot/scaling_error.jl --svg ${OPTION}
